# Evals — writing-skills:adams-plain-grade

## 1. Positive trigger (should load the skill)
> "Rewrite this insurance denial letter for our patients in plain English — someone with low
> reading skills should be able to understand it without help."

Expected: skill loads; the most important point comes first in one simple sentence; sentences
average 10–14 words in active voice; no litigated formulas ("pursuant to", "aforesaid",
doublets) survive the rewrite; any unavoidable technical term (e.g. "deductible") is said
once and explained immediately in plain words; the grade lands at 5th (8th only where
precision demands, stated as such); the meaning of the denial — what was denied, why, what
the patient can do next — is exactly preserved, not softened.

## 2. Near-miss (should NOT load this skill)
> "Draft the internal design doc for the reconciliation engine's new matching pass —
> engineering audience."

Expected: professional/technical internal writing for a jargon-fluent audience — no
plain-grade constraint applies; `writing-skills:adams-smart-brevity` owns this register.
Loading adams-plain-grade here means the audience boundary between the two Adams skills is
failing.

## 2b. Near-miss (precision-drafting guard)
> "Tighten the indemnification clause in this vendor contract — I want it precise and
> enforceable."

Expected: contract-drafting precision work — `writing-skills:adams-smart-brevity` owns it
(same Adams discipline, professional register, contract-language categories). This skill
should stay unloaded rather than push grade-level constraints onto operative legal drafting.
Doubly so here: an indemnity is a risk-allocation provision, which both Adams skills carve
out from rewriting. If this skill does load, it must explain the clause in plain words
*beside* it rather than simplify the operative wording, and must not trim "indemnify,
defend, and hold harmless" as a doublet — "defend" is a separate, often broader duty.

## 2c. Near-miss (explanation-design guard)
> "Our onboarding doc explains the deployment pipeline but new hires still don't get it —
> help me make the concept actually land."

Expected: the job is designing what the explanation must accomplish (audience model, entry
analogy, teach-back) — `writing-skills:explanation-design`. This skill changes the words'
register; that one designs what the words must do. Loading plain-grade here means the
family's design-vs-register seam is failing.

## 3. Quality rubric
- **Does**: main point first; short active sentences; everyday concrete words; one idea per
  sentence; technical terms glossed in place; grade verified 5th (or justified 8th, never
  higher); action steps, if any, given checklist form per
  `safety-and-reliability-skills:checklist-design` while the prose stays at grade.
- **Teaches**: why litigated language is bad language; why meaning always beats grade (the
  8th-grade fallback exists so precision never excuses complexity); why the read-aloud test
  outranks the readability score (the formulas measure only sentence and word length).
- **Stays honest**: meaning exactly preserved — no softening, no dropped qualifiers; anything
  that must appear verbatim (regulatory text) is quoted and then explained, not paraphrased;
  readability claims are computed or estimated per the reference's method, never invented.
