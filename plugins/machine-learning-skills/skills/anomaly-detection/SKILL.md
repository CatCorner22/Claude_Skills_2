...
2. **Start statistical on single features.** For one numeric field, flag points beyond a threshold: the
   ...
   and often enough — but branch on the degenerate case first, and note the two rules degrade at
   *different* points and in different ways (measured on a 10,000-row zero-inflated series):
   - **MAD goes to 0 just past 50% ties** — the median absolute deviation from the median is itself
     the median of a mostly-zero vector — and the robust z-score then divides by zero.
   - **IQR survives much longer: until the tied block reaches the 75th percentile (~75% ties).** At
     51%, 60% and 74% ties the IQR was still 2400, 1500 and 100. When it finally collapses, the
     failure is *not* `inf`/`nan`: Q1 = Q3 = 0 gives a fence of exactly [0, 0], and the rule then
     flags **every non-zero row** — a "100% of rows are anomalies" result that looks like a finding.
   Zero-inflated counts and refund series are where this bites. The fallbacks are in
   `references/methods-and-thresholds.md`.
