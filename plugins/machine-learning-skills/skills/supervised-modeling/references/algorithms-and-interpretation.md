...
|---|---|---|
| Feature scaling | Standardize before any penalty | The penalty acts on the coefficient, and a coefficient is an effect *per unit*, so the units decide who gets shrunk. Re-express a column so its *values* are 100× larger (dollars → cents) and its coefficient is 100× smaller, so L1 penalizes it 100× *less* and L2 10,000× less — the features punished hardest are the small-scale ones, whatever their signal. Symptom of skipping it: a feature you know matters drops out of the lasso path while a big-numbered irrelevant one survives, and re-expressing one column in different units changes the selected set |
...
| `num_leaves` (LightGBM) | 31, with `max_depth` as a cap | LightGBM grows **leaf-wise**, so `num_leaves` alone does not bound *depth*: measured on LightGBM 4.7 with defaults otherwise, `num_leaves=31` produced trees up to 17 levels deep (median 10) and `num_leaves=255` up to 32 — long and stringy, not balanced. `max_depth` is the guard for that, and it caps leaves at 2^depth in return (`max_depth=6` gave 64 leaves however high `num_leaves` was set), so any `num_leaves` above 2^`max_depth` is inert. Set both, and keep `num_leaves` below the 2^`max_depth` ceiling |
...
| **Permutation importance** | The validation-metric drop when X's information is destroyed, **for this fitted model** | What a model *retrained* without X would lose. Correlated features cover for each other during the shuffle, and the masking scales with the size of the correlated **group**: a pair at r ≈ 0.95 splits the drop roughly in half, while a cluster of six near-duplicates leaves every member near zero even though shuffling the whole cluster costs the model everything | Permute correlated groups together; when the answer matters, compare against a leave-one-covariate-out refit |
...
| **Heavy collinearity at interpretation time** | Coefficient signs flip between bootstrap refits; every permutation importance inside the correlated block sits near zero while the block as a whole is load-bearing | Interpret the correlated set as one block, or pick a single named representative and disclose the choice |
