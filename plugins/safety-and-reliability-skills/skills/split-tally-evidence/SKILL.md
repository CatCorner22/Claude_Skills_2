...
- Not for: reconciling two systems' outputs against each other →
  `cash-management-skills:bank-reconciliation` owns financial reconciliation and
  `accounting-skills:account-reconciliations` owns the ledger craft; this skill designs what
  each side should hold so later comparison is even possible.
- Not for: assembling the audit file and responding to auditor requests →
