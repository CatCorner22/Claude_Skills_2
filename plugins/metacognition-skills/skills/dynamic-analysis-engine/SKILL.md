...
- Treating the loop's own tests as confirmatory → a hypothesis generated by looking at the data
  and then tested on that same data has no protected error rate. Report it as exploratory,
  correct for how many tests were run (`data-analytics-bi-skills:statistical-inference` carries
  the multiplicity policy), or confirm it on data you held back.
...
a dirty tree. Copy it into your own project — `.claude/skills-env/dynamic-analysis-engine.private.md` works well — fill it in
